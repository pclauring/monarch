...
export const getMonstersByOwnerAsync = createAsyncThunk(
  "monster/getMonsters",
...
export const createMonsterAsync = createAsyncThunk(
  "monster/createMonster",
  async (monster: IMonster) => {
    const response = await monsterService.createMonster(monster);
    // The value we return becomes the `fulfilled` action payload
    return response;
  }
);
...
export const deleteMonsterAsync = createAsyncThunk(
  "monster/deleteMonster",
  async (_id: string) => {
    const response = await monsterService.deleteMonster(_id);
    // The value we return becomes the `fulfilled` action payload
    return response;
  }
);
...
export const monsterSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getMonstersByOwnerAsync.pending, (state) => {
        state.status = "loading";
      })
      .addCase(getMonstersByOwnerAsync.fulfilled, (state, action) => {
        state.status = "idle";
        state.monsters = action.payload.data;
      })
      .addCase(createMonsterAsync.pending, (state) => {
        state.status = "loading";
      })
      .addCase(createMonsterAsync.fulfilled, (state, action) => {
        state.status = "idle";
      ...
      .addCase(addTrainingAsync.fulfilled, (state, action) => {
        ...
        state.monsters.splice(index, 1, action.payload.data);
      })
      .addCase(deleteMonsterAsync.pending, (state) => {
        state.status = "loading";
      })
      .addCase(deleteMonsterAsync.fulfilled, (state, action) => {
        state.status = "idle";
        state.monsters = state.monsters.filter(
          (monster) => monster._id !== action.payload.data._id
        );
